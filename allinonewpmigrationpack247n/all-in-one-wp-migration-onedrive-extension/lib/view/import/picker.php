...
<div id="ai1wmoe-import-modal" class="ai1wmoe-modal-container" role="dialog" tabindex="-1">
		...
		<div class="ai1wmoe-file-browser">
			...
			<div class="ai1wmoe-file-list" v-if="items !== false && items.length > 0">
				<div class="ai1wmoe-file-item">
					<span class="ai1wmoe-file-name-header">
						<?php _e( 'Name', AI1WMOE_PLUGIN_NAME ); ?>
					</span>
					<span class="ai1wmoe-file-date-header">
						<?php _e( 'Date', AI1WMOE_PLUGIN_NAME ); ?>
					</span>
					<span class="ai1wmoe-file-size-header">
						<?php _e( 'Size', AI1WMOE_PLUGIN_NAME ); ?>
					</span>
				</div>
			</div>
			<ul class="ai1wmoe-file-list">
				<li v-for="item in items" v-on:click="browse(item)" class="ai1wmoe-file-item">
					<span class="ai1wmoe-file-name">
						<i v-bind:class="item.type | icon"></i>
						{{ item.name }}
					</span>
					<span class="ai1wmoe-file-date">{{ item.date }}</span>
					<span class="ai1wmoe-file-size">{{ item.size }}</span>
				</li>
				<li
					v-if="items !== false && items.length === 0"
					style="text-align: center; cursor: default;"
					class="ai1wmoe-file-item">
					<strong><?php _e( 'No folders or files to list. Click on the navbar to go back.', AI1WMOE_PLUGIN_NAME ); ?></strong>
				</li>
				<li class="ai1wmoe-file-info" v-if="num_hidden_files === 1">
					{{ num_hidden_files }}
					<?php _e( 'file is hidden', AI1WMOE_PLUGIN_NAME ); ?>
					<i class="ai1wm-icon-help" title="<?php _e( 'Only wpress backups are listed', AI1WMOE_PLUGIN_NAME ); ?>"></i>
				</li>
				<li class="ai1wmoe-file-info" v-if="num_hidden_files > 1">
					{{ num_hidden_files }}
					<?php _e( 'files are hidden', AI1WMOE_PLUGIN_NAME ); ?>
					<i class="ai1wm-icon-help" title="<?php _e( 'Only wpress backups are listed', AI1WMOE_PLUGIN_NAME ); ?>"></i>
				</li>
			</ul>
		</div>
	...
	<div class="ai1wmoe-modal-legend">
		<p style="box-shadow: 0px -1px 1px 0px rgb(221, 221, 221);" class="ai1wmoe-file-info" v-if="items !== false">
			<?php _e( 'Open with a click', AI1WMOE_PLUGIN_NAME ); ?>
		</p>
	</div>

	<div class="ai1wmoe-modal-action">
		<transition>
			<p class="ai1wmoe-selected-file" v-if="file">
		...
		<p class="ai1wmoe-justified-container">
			...
			<button type="button" class="ai1wm-button-green" v-if="file" v-on:click="restore(file)">
				...
				<?php _e( 'Import', AI1WMOE_PLUGIN_NAME ); ?>
			</button>
		</p>
	</div>
</div>

<div id="ai1wmoe-import-overlay" class="ai1wmoe-overlay"></div>
